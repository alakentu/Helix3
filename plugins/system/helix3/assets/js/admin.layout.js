...
jQuery(function($){
	...
	$(document).on("click", ".layout-del-action", function (event) {
		...
		if (confirm(Joomla.Text._('HELIX_DELETE_LAYOUT_NAME').replace('%s', layoutName)) != true) {
			return false;
		}

		if (data.action != "remove") {
			alert(Joomla.Text._('HELIX_YOU_DOING_SOMETHINGS_WRONG'));
		}
	...
	$(document).on("click", ".layout-save-action", function (event) {
		...
		$("#layout-modal .sp-modal-title").text(Joomla.Text._('HELIX_SAVE_NEW_LAYOUT'));
	...
	$(document).on("click", ".row-ops-set", function (event) {
		...
		$("#layout-modal").find(".sp-modal-body").empty();
		$("#layout-modal .sp-modal-title").text(Joomla.Text._('HELIX_ROW_SETTINGS'));
	...
	$(document).on("click", ".col-ops-set", function (event) {
		...
		$("#layout-modal").find(".sp-modal-body").empty();
		$("#layout-modal .sp-modal-title").text(Joomla.Text._('HELIX_COLUMN_SETTINGS'));
	...
	$(".input-column_type").change(function (event) {
		...
		if (flag) {
			alert(Joomla.Text._('HELIX_AREA_COMPONENT_TAKEN'));
	...
	$(document).on("click", "#save-settings", function (event) {
		...
		switch (flag) {
					...
					.each(function () {
						...
						if ($attrname == "name") {
							...
							if (nameVal != "" || $this.val() != null) {
								...
								$(".row-active .section-title").text(Joomla.Text._('HELIX_SECTION_HEADER'));
							}
						}
				...
				$("#layout-modal")
					...
					.each(function () {
						...
						if ($attrname == "column_type" && $(this).attr("checked")) {
							component = true;
							$(".column-active .col-title").text(Joomla.Text._('HELIX_COMPONENT'));
			...
			case "save-layout":
				...
				if (layoutName == "" || layoutName == " ") {
					alert(Joomla.Text._('HELIX_ALERT_LAYOUT_NO_NAME_NOT_SAVE'));
					return false;
				}
	...
	$(document).on("click", ".column-layout", function (event) {
		...
		if (colType == "custom") {
			column = prompt(Joomla.Text._('HELIX_ENTER_CUSTOM_LAYOUT_GRID'), "4,2,2,2,2");
		}
		...
		if (colType == "custom") {
			...
			if (error) {
				alert(Joomla.Text._('HELIX_ERROR_GENERATED'));
				return false;
			}
		}
		...
		for (var i = 0; i < newLayout.length; i++) {
			...
			if (col[i]) {
				...
				new_item +=
					'<div class="column"> <h6 class="col-title pull-left">'+Joomla.Text._('HELIX_NONE')+'</h6> <a class="col-ops-set pull-right" href="#" ><i class="fa fa-gears"></i></a></div>';
			}
			new_item += "</div>";
		}
	...
	$(document).on("click", ".remove-row", function (event) {
		...
		if (confirm(Joomla.Text._('HELIX_CLICK_OK_TO_DELETE_ROW')) == true) {
			$(this)
